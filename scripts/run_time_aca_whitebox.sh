...
gen_job(){
    echo "$1 --key_file=$2 --dataset_type=$3 --init_num_keys=$4 --workload_size=$5 --c=$6 --budget=$7 --lookup_perc=$8" >> $job_description
}

local(){
    /home/ubuntu/sofs/parallel-20230122/src/parallel \
        --group --resume-failed -j 85% --load 75% --progress \
...
run_exps(){
    ...
    output_file="/home/ubuntu/data/attack_out/out/perf_aca_greedy_whitebox.out"
            ...
            for c in ${c_list[@]}; do
                for b in ${b_list[@]}; do
                    for lookperc in ${lookup_perc_list[@]}; do
                        ...
                        for (( j=1; j<=$sample_run; j++)); do
                            gen_job $cpp_bin $in_file $data_type $init_num_keys $size $c $b $lookperc
                        done
