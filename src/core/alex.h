             ...
             bool allow_duplicates = true>
                 class Alex {
                     ...
                     struct Stats {
                         ...
                         /*Rui: usefull for telling the stats about expansion */
                         int sideways_split_root_expands = 0;
                         int num_actions_on_root = 0;

                         int num_cost_deviation_splits = 0;
                         int num_catasctrophic_splits = 0;
                         int num_exceed_max_capcity_splits = 0;
                     };
                     Stats stats_;
                     ...
                     public:
                     /*Rui: varible for get the RMI depth*/
                     short max_level = 0;
                     ...
                     //Rui:
                     //use to compute memory consumption for a datanode(keys, payloads, bitmap arrays)
                     //return memory in bytes

                     int compute_reserved_memory(data_node_type* leaf) {
                         return sizeof(T) * leaf->data_capacity_ + 
                             sizeof(P) * leaf->data_capacity_ + sizeof(uint64_t) * leaf->bitmap_size_;
                     }



                     // compute the memory used to store the keys, payloads, and bitmap
                     double compute_running_memory(data_node_type* leaf) {

                         double reserved_bitmap_bit = sizeof(uint64_t) * leaf->bitmap_size_ * 8;
                         double consumed_bitmap_bit = leaf->num_keys_ / reserved_bitmap_bit;
                         double consumed_bitmap_bytes = consumed_bitmap_bit / 8;


                         return sizeof(T) * leaf->num_keys_ +
                             sizeof(P) * leaf->num_keys_ + consumed_bitmap_bytes;
                     }

                     model_node_type* get_parent(data_node_type* node)
                     {
                         std::vector<TraversalNode> traversal_path;
                         T key = node->first_key();
                         get_leaf(key, &traversal_path);

                         return traversal_path.back().node;
                     }
                     ...
                     void bulk_load(const V values[], int num_keys) {
                         ...
                         /* Rui's thoughts:
                          * model_.a: slope 
                          * model_.b: intercept b = y - m * x 
                          * 
                          * Below two lines of code is calculating the slope and intecept using 
                          * point_1(min_key, 0) and point_2(max_key, 1).
                          *
                          * */
                         ...
                         /*Rui's thoughts:
                          *Below two lines of code compute the root node's model until the a(m:slope)&&b(b:y-intercept) 
                          *changes converged.
                          * */
                     ...
                     void bulk_load_node(const V values[], int num_keys, AlexNode<T, P>*& node,
                         ...
                         // than current cost
                         // Rui: derived_params_{max_fanout_size, max_data_slots} is deafult strcut 
                         ...
                         if (experimental_params_.fanout_selection_method == 0) { // bottom-up
                         ...
                         } else if (experimental_params_.fanout_selection_method == 1) { //top-down
                     ...
                     // use lower_bound()
                     typename self_type::Iterator find(const T& key, FindBenchmarkStruc *data=nullptr) {
                         stats_.num_lookups++;
                         data_node_type* leaf = get_leaf(key);
                         int idx = leaf->find_key(key, data);
                         data->depth = leaf->level_;
                     ...
                     // found.
                     std::pair<Iterator,bool> insert(const T& key, const P& payload,  ExpeData *l=nullptr) {
                         ...
                         int predict_pos = leaf->predict_position(key);
                         int fail = ret.first;
                         ...
                         if (fail) {
                             ...
                             while (fail) {
                                 ...
                                 //changed it to == for LIS attack test
                                 if (experimental_params_.splitting_policy_method == 0 || fail >= 2) {
                                     ...
                                     fanout_tree_depth = fanout_tree::find_best_fanout_existing_node<T, P>(
                                             parent, bucketID, stats_.num_keys, used_fanout_tree_nodes, 2, l);
                                 ...
                                 if (fanout_tree_depth == 0) {
                                     ...
                                     if (experimental_params_.allow_splitting_upwards) {
                                         ...
                                         // Rui:
                                         // 1) parent->num_children_ * best_fanout / (1 << leaf->duplication_factor_): this 
                                         //  should be the total nodes of the parent node after choose to split in sideways
                                         //  we compare this with max_fanout to decide 
                                         //  if we should split sideways or downards 
                                         ...
                                         if (should_split_downwards) {
                                             ...
                                             if(parent->children_[bucketID]->duplication_factor_ < 1) {
                                                 data_node_type* d0= static_cast<data_node_type*> (parent->children_[bucketID]);
                                             }
                     ...
                     /***take a snapshot of the all datanodes of  alex tree ***/
                     void datanode_snapshot(double consumed_memos[], 
                             double reserved_memos[], 
                             int levels[], 
                             int left_keys[], 
                             int insert_keys[],
                             int progress)
                     {
                         ...
                         //link all data nodes doesn't matter if the depths are different
                         //link_all_data_nodes();
                         //get the most left data node
                         cur = first_data_node();
                         //std::cout<<"size T =" << sizeof(T) << "size P"<<sizeof(P) <<"\n";
                         //sizeof(T);

                         int cumulative_nodes_ = 0;
                         while(cur) {
                             consumed_memos[cnt] = compute_running_memory(cur);
                             reserved_memos[cnt] = compute_reserved_memory(cur);
                             levels[cnt] = cur->level_;
                             left_keys[cnt] = cur->data_capacity_ - cur->num_keys_;
                             insert_keys[cnt] = cur->num_keys_;

                             /*
                                if(consumed_memos[cnt] / reserved_memos[cnt] > 0.9) {
                                cumulative_nodes_++;
                                std::cout<<"progress=" << progress 
                                <<" capacity="<<cur->data_capacity_
                                <<" num_keys"<<cur->num_keys_ 
                                <<" num_nodes_0.9="<<cumulative_nodes_
                                <<" key@4="<<cur->key_slots_[3]
                                <<" key@5="<<cur->key_slots_[4]
                                <<"\n";

                                }*/
                             //std::cout<<"node_number="<< cnt
                             //         <<" consumed=" <<consumed_memos[cnt] 
                             //         <<" reserved=" << reserved_memos[cnt]
                             //         << " level="<<cur->level_<<"\n";
                     ...
                     void datanode_snapshot_timeline(double consumed_memos[],
                             double reserved_memos[])
                     {
                         int cnt = 0;
                         data_node_type* cur;
                     ...
                     void print_max_input_size_datanodes(int size)
                     {
                         int cnt = 0;
                         data_node_type* cur;

                         std::vector<std::pair<int, data_node_type*>> dns_descending;
                         //link all data nodes doesn't matter if the depths are different
                         //link_all_data_nodes();
                         //get the most left data node
                         cur = first_data_node();
                         //std::cout<<"size T =" << sizeof(T) << "size P"<<sizeof(P) <<"\n";
                         //sizeof(T);
                         std::pair<int , data_node_type*> p;
                         while(cur) {
                             p.first = compute_reserved_memory(cur);
                             p.second = cur;
                             dns_descending.push_back(p);
                             //update cur and cnt
                             cur = cur->next_leaf_;
                             cnt++;
                         }

                         std::sort(dns_descending.begin(), dns_descending.end(), [](const std::pair<int, data_node_type*> &a,
                                     const std::pair<int, data_node_type*> &b) {
                                 return a.first > b.first;});

                         for(int i = 0; i < size; i++) {
                             std::cout<<"datanode_allocated_memory="<<dns_descending.at(i).first
                                 << "|  datanode_num_keys="<<dns_descending.at(i).second->num_keys_
                                 << "|  datanode_num_expends="<<dns_descending.at(i).second->num_resizes_
                                 <<std::endl;
                             dns_descending.at(i).second->print_all_keys(i);
                         }
                     }


                     void get_all_datanodes(std::vector<data_node_type*> &vec) {

                         data_node_type* cur = first_data_node();

                         if (stats_.num_data_nodes == 1){
                             vec.push_back(cur); 
                             return;
                         }
                     ...
                     model_node_type* split_downwards(
                             ...
                             std::vector<fanout_tree::FTNode>& used_fanout_tree_nodes,
                             bool reuse_model, ExpeData* data=nullptr) {
                     ...
                     void split_sideways(model_node_type* parent, int bucketID,
                         ...
                         if (fanout > repeats) {
                             ...
                             if(root_node_ == parent) {
                                 stats_.sideways_split_root_expands++; 
                             }
                     ...
                     void create_new_data_nodes(
                         ...
                         for (fanout_tree::FTNode& tree_node : used_fanout_tree_nodes) {
                             ...
                             reserved_mem += compute_reserved_memory(child_node);
                             cur += child_node_repeats;
                         ...
                         //int old_datanode_mem = compute_reserved_memory((data_node_type*) old_node);
                         //std::cout<<"old_datanode_mem="<< old_datanode_mem
                     ...
                     std::vector<long long> get_model_sizes() const {
                                 ...
                                 : index_(index), cur_node_(index->root_node_) {
                                     if (cur_node_ && !cur_node_->is_leaf_) {
                                         ...
                                         for (int i = node->num_children_ - 2; i >= 0; i--) {
                                             ...
                                             //this if statement voids duplicate nodes added to stack.
                                             if (node->children_[i] != node->children_[i + 1]) {
