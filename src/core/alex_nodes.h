             ...
             bool allow_duplicates = true>
                 class AlexDataNode : public AlexNode<T, P> {
                     public:
                         ...
                         void print_all_keys(int datanode_num) const {
                             std::ofstream outputFile;
                             outputFile.open("/root/data/ALEX/attack/keys_at_" + std::to_string(datanode_num));
                             std::cout<<"===========start printing datanode keys=============\n";  
                                 for (int i = 0; i < data_capacity_; i++) {
                                     if (check_exists(i)) {
                                         outputFile<<"positio= "<< i
                                             <<" key= "<<get_key(i)
                                             <<std::endl;
                                     }
                                 }
                             std::cout<<"===========end printing datanode keys=============\n";
                         ...
                         static double compute_expected_cost(
                             ...
                             /*Rui: this cost should be the cost for intra-node cost. Which is the cost for error(pre_position, acu_postion) and the cost for the shifting keys
                              *
                              * */
                         ...
                         static void build_node_implicit(const V* values, int num_keys,
                             ...
                             for (int i = 0; i < num_keys; i++) {
                                 ...
                                 //std::cout<<"predicted_position= "<<predicted_position<<", actual_position=a"<< actual_position <<", positions_remaining= " << positions_remaining <<", data_capacity="<<data_capacity<<"key_remanining="<<keys_remaining<<std::endl;

                                 /*Rui's thoughts:
                                  * why do we need to check if positions_remaning < keys_remaining?
                                  * My guess is that since the data_capacity = num_keys / desired dencity. The data_capacity is the acutal
                                  * gapped array size we should have for a node. 
                                  * positions_remaining >= keys_remaning at all time if the prediction is close to acurrated. 
                                  * If
                                  *
                                  * */
                         ...
                         static double compute_expected_cost_from_existing(
                             ...
                             if (c) {
                                c->iters_cost = kExpSearchIterationsWeight * expected_avg_exp_search_iterations;
                                c->shifts_cost = kShiftsWeight * expected_avg_shifts * expected_insert_frac;
                             }
                         ...
                         static void build_node_implicit_from_existing(const self_type* node, int left,
                             ...
                             for (; it.cur_idx_ < right && !it.is_end(); it++) {
                                 ...
                                 //Rui add
                                 if( c && it.key() == c->single_poison_key && predicted_position != actual_position){
                                    c->occup = 1;
                                 }
                         ...
                         static void build_model_sampling(const V* values, int num_keys,
                             ...
                             for (int i = 0; i < num_keys; i += step_size) {
                                 builder.add(values[i].first, i);
                                 //std::cout<<"(x,y): ("<< values[i].first<<","<<i<<")"<<std::endl;
                             }
                             builder.build();
                         ...
                         std::vector<int> find_key(const T& key) {
                             num_lookups_++;
                             ...
                             //Rui:
                             int its_before_search = num_exp_search_iterations_;
                             int pos = exponential_search_upper_bound(predicted_pos, key) - 1;
                             
                             //Rui
                             int num_its = num_exp_search_iterations_ - its_before_search;
                             ...
                             if (pos < 0 || !key_equal(ALEX_DATA_NODE_KEY_AT(pos), key)) {
                                 ...
                                 res.push_back(pos);
                                 res.push_back(std::abs(pos - predicted_pos));
                                 res.push_back(num_its);
                         ...
                         // -1 if no insertion.
                         std::pair<int, int> insert(const T& key, const P& payload, ExpeData *e = NULL) {
