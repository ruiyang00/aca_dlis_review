...
int main(int argc, char* argv[]) {
    ...
    int init_num_keys;
    if(init_perc == 0) {
        init_num_keys = total_num_keys - num_poisoning_keys;
    } else {
    ...
    for(int i = 0; i < poisoning_keys.size(); i++) {
        cout<<poisoning_keys[i]<<"\n";
    }

    if(index.stats_.num_keys < total_num_keys) {
        attack::insert_left_keys(dns,
                index,
                total_num_keys - index.stats_.num_keys,
                poisoning_keys,
                chosen_node_density,
