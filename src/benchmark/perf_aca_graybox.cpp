...
int main(int argc, char* argv[]) {
    ...
    auto sub_key_file = get_required(flags, "sub_key_file");
    ...
    alex::Alex<KEY_TYPE, PAYLOAD_TYPE> index, sub_index; 
    ...
    for(int i = 0; i < num_batch; i++) {
        ...
        while(running_posi_inserts < num_poison_keys_per_batch) {
            ...
            dns.clear();
            sub_index.get_all_datanodes(dns);
            std::sort(dns.begin(),dns.begin() + dns.size(),
            ...
            for(int j = 0; j < poisoning_keys.size(); j++) {
                sub_index.insert(poisoning_keys[j], static_cast<PAYLOAD_TYPE>(gen_payload()));
            }
